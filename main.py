...
logging.basicConfig(level=logging.DEBUG)  # Измените на DEBUG для подробных логов
...
# Обновите функцию get_subjects_keyboard:
def get_subjects_keyboard(teacher_key: str):
    ...
    if teacher:
        ...
        for subject in subjects:
            # УБЕДИТЕСЬ, что хэш вычисляется одинаково везде
            sub_hash = get_subject_hash(subject)
            logger.debug(f"Создание кнопки: предмет={subject}, хэш={sub_hash}")
            buttons.append([InlineKeyboardButton(
                text=subject,
...
def get_dates_keyboard(teacher_key: str, subject_hash: str, subject_name: str):
    ...
    if teacher:
        for subj in teacher["subjects"]:
            current_hash = get_subject_hash(subj)
            logger.debug(f"Сравнение хэшей: {current_hash} == {subject_hash} ?")
            if current_hash == subject_hash:
                subject_found = subj
                logger.debug(f"Предмет найден: {subject_found}")
                break

    if not subject_found:
        # Попробуем найти по точному имени
        if teacher:
            for subj in teacher["subjects"]:
        ...
        if not subject_found:
            logger.error(f"Предмет не найден! Хэш: {subject_hash}, Имя: {subject_name}")
            logger.error(f"Все предметы учителя {teacher_key}: {teacher['subjects'] if teacher else 'Нет учителя'}")
